...
import { homeQuery } from '@/app/api/api';
import { useEffect, useState } from "react";
import React from "react";

export default function ListFooterEnd({ pathname, urlHome = "", openModal = () => { } }) {
    const { data, error, isLoading } = useSWR('home', homeQuery);
    ...
    return (
        <>
            {(!isLoading && data) && <>
                <div className={`border-t-[0.75px] border-t-astro-gray py-6 md:py-28 ${(pathname == "/contact/" ? "hidden md:block" : "")} `}>
                    <div className="container">
                        <div className="flex md:hidden justify-center text-center">
                            <Link href="/contact" className=" inline-block py-2 text-king-red font-btn uppercase">Get in touch</Link>
                        </div>
                        <div className="hidden md:grid md:grid-cols-3 md:gap-8">
                            <div className="flex flex-col items-start">
                                <LinkFooter urlHome={urlHome} text="restaurants" to="restaurants" properties="mb-4" />
                                {
                            ...
                            <div className="flex flex-col items-start">
                                <LinkFooter urlHome={urlHome} text="classes and tastings" to="classes-and-tastings" properties="mb-4" />
                                {
                            ...
                            <div className="flex flex-col items-start">
                                <LinkFooter urlHome={urlHome} text="experiences" to="experiences" properties="mb-4" />
                                {
                        ...
                        <div className="hidden md:grid md:grid-cols-3 md:gap-8">
                            <div className="flex flex-col items-start">
                                <LinkFooter urlHome={urlHome} text="special events" to="special-events" properties="mb-8" />
                                <LinkFooter urlHome={urlHome} text="rituals" to="rituals" />
                            </div>
                            <div className="flex flex-col items-start">
                                <LinkFooter urlHome={urlHome} text="weekly activities" to="weekly-activities" properties="mb-8" />
                                <LinkFooter urlHome={urlHome} text="Children’s Club" to="children-club" />
                            </div>
                            <div className="flex flex-col items-start">
                                <LinkFooter urlHome={urlHome} text="spa" to="spa" properties="mb-8" />
                                <Link href={"/contact"} className="link-main-footer link-hover-line red-king">Contact</Link>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="container">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-12 text-center md:text-left">
                            <div className="flex flex-col items-center md:items-start py-2">
                                <Link href={"/terms-and-conditions"} className="link-footer">Terms and Conditions</Link>
                            </div>
                            <div className="flex flex-col items-center md:items-start py-2">
                                <Link href={"/notice-of-privacy"} className="link-footer">Notice of Privacy</Link>
                            </div>
